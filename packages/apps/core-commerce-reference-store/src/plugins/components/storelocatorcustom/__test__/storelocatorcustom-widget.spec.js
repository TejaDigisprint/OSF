...
export const storelocatorcustom = () => import('../index');

describe('storelocatorcustom renders correctly and shows after changing visible state', () => {
  /**
   * Reference to dynamic imported comps to resolve and preload.
   */
  const comps = {storelocatorcustom};
  beforeAll(async () => {
    /**
  ...
  it('storelocatorcustom works as expected', async done => {
    /**
    ...
    const state = {
      pageRepository: {
        widgets: {
          dsWidgetInstance: {
            id: 'dsWidgetInstance',
            componentId: 'storelocatorcustom',
            regions: []
          }
        }
      }
