...
export const CommerceItemTotalPrice = props => {
  ...
  let discountPriceInfo = null;
  let actualPriceInfo = null;
  ...
  if (
    ...
    discountPriceInfo = !isNaN(shippingGroupCommerceItem.discountAmount) && formatCurrency(shippingGroupCommerceItem.discountAmount);
    actualPriceInfo = !isNaN(shippingGroupCommerceItem.rawTotalPrice) && formatCurrency(shippingGroupCommerceItem.rawTotalPrice);
  } else if (
    commerceItem.giftWithPurchaseCommerceItemMarkers &&
    commerceItem.giftWithPurchaseCommerceItemMarkers.length > 0 &&
  ...
  // let savePriceInfo=formatCurrency((shippingGroupCommerceItem.amount-commerceItem.rawTotalPrice).toFixed(2))
 
  return (
    <Styled id="CommerceItemTotalPrice" css={css}>
      <div className="CommerceItemTotalPrice">{totalPriceInfo}</div>
      <div className='commerceItemDiscountprice'>Save:{formatCurrency(Math.abs((commerceItem.rawTotalPrice-shippingGroupCommerceItem.amount)).toFixed(2))}</div>
      <div className='commerceItemActualprice'>Was:{actualPriceInfo}</div>
    </Styled>
  );
};
...
export const CommerceItemSkuProperties = (props) => {
  const skuProperties = props && props.skuProperties || [];
  const showSkuProperties = JSON.parse(((props && props.showProductPropertiesInCart) || "").replace(/'/g, '"')) || []

  return (
    <Styled id="CommerceItemProperties" css={css}>
      <div className="CommerceItemProperties">
        {skuProperties &&
          skuProperties.map(item => (
            <>
            {showSkuProperties.includes(item.name) ?
            <div key={item.name}>
