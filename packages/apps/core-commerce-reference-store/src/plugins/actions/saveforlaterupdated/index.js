...
function* saveforlaterupdatedSaga(action) {
  yield put({type:'saveforlater',payload:action.payload});
}

function* saveforlaterremoveSaga(action) {
  yield put({type:'saveforlaterremove',payload:action.payload});
}
/**
 * The saveforlaterupdated action.
 *
...
export default {
  *saga() {
    yield all([takeEvery('saveforlaterupdated', saveforlaterupdatedSaga),
  takeEvery("saveforlaterremover",saveforlaterremoveSaga)]
    )}
};
